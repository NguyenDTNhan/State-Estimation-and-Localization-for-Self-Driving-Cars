...
Qk = np.identity(6)
Qk[0:3,0:3] = Qk[0:3,0:3]*var_imu_f
Qk[3:6,3:6] = Qk[3:6,3:6]*var_imu_w
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    K_k = p_cov_check@(h_jac.T)@np.linalg.inv(h_jac@p_cov_check@(h_jac.T)+sensor_var*np.identity(3))

    # 3.2 Compute error state
    doe_x = K_k@(y_k-p_check)
    # 3.3 Correct predicted state
    p_hat = p_check + doe_x[0:3]
    v_hat = v_check + doe_x[3:6]
    q_hat = Quaternion(axis_angle=doe_x[6:9]).quat_mult_left(q_check, out='Quaternion')
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    tmp = delta_t*(np.dot(C_ns,imu_f.data[k-1])+g)
    p_check = p_est[k-1] + delta_t*v_est[k-1] + delta_t/2*tmp
    v_check = v_est[k-1] + tmp
    q_check = Quaternion(axis_angle=imu_w.data[k-1]*delta_t).quat_mult_right(q_est[k-1], out='Quaternion') #Why axis_angle instead of euler_angle?

    # 1.1 Linearize the motion model and compute Jacobians
    f_jac[0:3,3:6] = np.identity(3)*delta_t
    f_jac[3:6,6:9] = -skew_symmetric(np.dot(C_ns,imu_f.data[k-1])) * delta_t
    # 2. Propagate uncertainty
    p_cov_check = f_jac@p_cov[k-1]@(f_jac.T) + l_jac@(delta_t**2*Qk)@(l_jac.T)
    # 3. Check availability of GNSS and LIDAR measurements

    if i_lidar < lidar.t.size and imu_f.t[k] >= lidar.t[i_lidar]:
        p_check, v_check, q_check, p_cov_check = measurement_update(var_lidar,p_cov_check,lidar.data[i_lidar],p_check,v_check,q_check)
        i_lidar+=1

    if i_gnss < gnss.t.size and imu_f.t[k] >= gnss.t[i_gnss]:
        p_check, v_check, q_check, p_cov_check = measurement_update(var_gnss,p_cov_check,gnss.data[i_gnss],p_check,v_check,q_check)
        i_gnss+=1
